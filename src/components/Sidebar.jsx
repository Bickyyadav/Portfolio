...
export default function Sidebar({isOpen, setIsOpen}) {
  ...
  return (
    <div
      className={`bg-slate-100 h-screen p-4 flex flex-col justify-between transition-all duration-300
      ${isOpen ? 'w-64' : 'w-20'} fixed md:relative`}
    >
      <button onClick={() => setIsOpen(!isOpen)} className="absolute top-4 right-[-12px] p-2 bg-gray-200 rounded-full shadow-md transition-all">
      ...
      <div className="mt-10">
        ...
        <nav className="mt-6 space-y-2">
          {navbar.map(item => (
            <NavLink to={item.path} key={item.name} className={`flex items-center p-2 text-gray-700 hover:bg-slate-300 rounded-md transition ${path.pathname === item.path ? 'bg-slate-300' : ''}`}>
      ...
      {isOpen && (
        <NavLink to="https://resume.io/resume-examples/driver" target="_" className="bg-black text-white px-4 py-2 rounded-full shadow-md hover:bg-gray-800 transition text-center">
          Read Resume →
        </NavLink>
      )}
    </div>
